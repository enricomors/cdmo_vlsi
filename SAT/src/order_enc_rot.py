...
def order_enc_rot(instance, index, args):
    ...
    def no_overlap_x_rot(i, j):
        res = []
        res.append([Not(px[j][y[i] - 1])])
        for e in range(w - y[i] - 1):
            res.append([px[i][e], Not(px[j][e + y[i]])])
        res.append([px[i][w - y[i] - 1]])
        return res
    ...
    def no_overlap_y_rot(i, j):
        res = []
        res.append([Not(py[j][x[i] - 1])])
        for f in range(maxh - x[i] - 1):
            res.append([py[i][f], Not(py[j][f + x[i]])])
        res.append([py[i][maxh - x[i] - 1]])
        return res
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            # rotation:
            for pr in no_overlap_x_rot(i, j):
                prop = [Not(lr[i][j])] + pr
                s.add(Implies(R[i], Or(prop)))

            for pr in no_overlap_x_rot(j, i):
                prop = [Not(lr[j][i])] + pr
                s.add(Implies(R[i], Or(prop)))

            for pr in no_overlap_y_rot(i, j):
                prop = [Not(ud[i][j])] + pr
                s.add(Implies(R[i], Or(prop)))

            for pr in no_overlap_y_rot(j, i):
                prop = [Not(ud[j][i])] + pr
                s.add(Implies(R[i], Or(prop)))
